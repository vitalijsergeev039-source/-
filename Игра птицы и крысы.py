...
pygame.display.set_caption("Птицы и крысы — прозрачные спрайты")
...
def spawn_rat():
    if len(rats) < 1:
        x = random.randint(WIDTH + 50, WIDTH + 500)
        y = PLAYER_Y_GROUND + 30
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not game_over and not game_won:
                ...
                # Прыжок
                if event.key == pygame.K_w and abs(player_y - PLAYER_Y_GROUND) < 1:
    ...
    if not game_over and not game_won:
        ...
        if now - last_bird_spawn > BIRD_SPAWN_INTERVAL:
            spawn_bird()
            last_bird_spawn = now

        if now - last_rat_spawn > RAT_SPAWN_INTERVAL:
            spawn_rat()
            last_rat_spawn = now
    ...
    screen.blit(FONT.render(f"Удары: {hits_taken}/{MAX_HITS}", True, (255, 100, 100)), (10, 40))
